...
import 'package:trip_logger/analytics_page.dart';
...
class _BusappState extends State<Busapp> {
  ...
  void perBus(String? busno) async {
    if (busno != null) {
      List<Map<String, dynamic>> summary = await dbHelper.previouslyTraveled(
        busno,
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Row(children: [Text("Bus app "), Icon(Icons.alt_route_sharp)]),
        actions: [
          IconButton(
            ...
            onPressed: () {
              Navigator.of(
                context,
              ).push(MaterialPageRoute(builder: (_) => AnalyticsPage()));
            },
          ),
          IconButton(
              ...
              if (busno != null) {
                Navigator.of(context)
                    .push(
                      MaterialPageRoute(
                        builder:
                            (context) => AddTripForm(busno: busno as String),
                      ),
                    )
                    .then((_) => setS());
              }
                  ...
                  Navigator.of(context)
                      .push(
                        MaterialPageRoute(
                          builder:
                              (context) =>
                                  Buslist(routeName: item["route_name"]),
                        ),
                      )
                      .then((_) => setS());
                },
